const contractAddress = "0x5b7d8052d8993312bf5d12ea07e69606AE73063e";
const abi = [
  {
    inputs: [
      {
  ...
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "_totalTeamtoken",
        type: "uint256",
      },
      {
        internalType: "uint256",
        name: "_totalPrivateSaleToken",
        type: "uint256",
      },
      {
        internalType: "uint256",
...
async function publicSaleTransfer() {
  ...
  const weiAmount = ethers.utils.parseEther(amount);
  try {
    const tx = await contract.publicSaletransfer({ value: weiAmount });
    await tx.wait();
    alert("Transaction successful!");
  } catch (error) {
    console.error("Error:", error);
...
async function privateSaleTransfer() {
  const amount = document.getElementById("privateSaleAmount").value;
  const weiAmount = ethers.utils.parseEther(amount);
  try {
...
async function teamSaleTransfer() {
  try {
    const tx = await contract.teamSaletransfer();
    await tx.wait();
    alert("Transaction successful!");
  } catch (error) {
    console.error("Error:", error);
...
async function airdropTokens() {
  const addresses = document
    .getElementById("airdropAddresses")
    .value.split(",");
  const amount = document.getElementById("airdropAmount").value;
  console.log(amount);
  const weiAmount = ethers.utils.parseEther(amount);
  console.log(addresses, weiAmount);
  try {
    const tx = await contract.airDropToken(addresses, weiAmount);
    await tx.wait();
...
async function setTeamAddresses() {
  ...
  try {
    const tx = await contract.setTeamAddress(addresses);
    await tx.wait();
    alert("Team addresses set successfully!");
  } catch (error) {
    console.error("Error:", error);
  }
}

async function setRewardLimit() {
  const amount = document.getElementById("rewardLimitAmount").value;
  try {
    const tx = await contract.setRewardLimit(amount);
    await tx.wait();
    alert("Reward limit set successfully!");
  } catch (error) {
    console.error("Error:", error);
  }
}
